# Briefing: Tradução DE → NL — Site Astro Estático

> **Objetivo:** Traduzir todo o conteúdo alemão para holandês, preservando 100% do design, layout, componentes e imagens neutras. Imagens com texto embutido em alemão devem ser sinalizadas para substituição manual.
...
find . -name "*.json" | xargs grep -l "\"de\"\|\"nl\"\|translations\|messages" 2>/dev/null | grep -v node_modules
...
| Cenário | Indicador |
|---|---|
| **A** — Pastas duplicadas | `src/pages/de/` e `src/pages/nl/` existem |
| **B** — i18n oficial Astro | `i18n` configurado em `astro.config.*` com `locales` |
| **C** — Arquivos de tradução JSON/TS | Arquivos como `de.json`, `nl.json`, `translations.ts` |
...
### Cenário A — Pastas `/de/` e `/nl/` duplicadas

1. Compare `src/pages/de/` com `src/pages/nl/` para identificar arquivos ausentes no NL
2. Para cada arquivo presente em `/de/` mas ausente em `/nl/`: copie o arquivo e traduza
3. Para arquivos já existentes em `/nl/`: revise e complete a tradução
...
diff <(ls src/pages/de/) <(ls src/pages/nl/)
```
...
3. Copie o arquivo `de` como base para o `nl` e traduza todas as strings
4. Não altere `astro.config.*` — a rota `/nl/` já deve estar configurada

### Cenário C — Arquivos de tradução JSON/TS

1. Localize os arquivos de tradução (ex: `src/i18n/de.json`, `src/locales/de.ts`)
2. Copie o arquivo alemão como base NL: `cp src/i18n/de.json src/i18n/nl.json`
3. Traduza todos os valores (nunca as chaves) de alemão para holandês
4. Verifique se o arquivo NL está sendo importado/registrado no sistema de tradução
...
```
2. Para cada arquivo: substitua o texto alemão pelo holandês equivalente
...
### 🇳🇱 Qualidade da tradução holandesa
- Use holandês europeu padrão (Nederlanden — não afrikaans)
...
Crie o arquivo `NL_IMAGES_TODO.md` na raiz do projeto com:
...
|---|---|---|---|
| /public/images/banner-hero.jpg | src/pages/index.astro | "Willkommen" | Recriar em NL |
| ... | ... | ... | ... |
```
...
```astro
{/* TODO-NL: Imagem contém texto em alemão — substituir por versão holandesa */}
<img src="/images/banner.jpg" alt="Hoofdbanner" />
```
...
```astro
<!-- Exemplo do que deve ser ajustado em cada página NL -->
<head>
  <title>Paginatitel — Sitenaam</title>
  <meta name="description" content="Beschrijving in het Nederlands..." />
  <meta property="og:title" content="Paginatitel" />
  <meta property="og:description" content="Beschrijving in het Nederlands..." />
  <meta property="og:locale" content="nl_NL" />
  <link rel="alternate" hreflang="de" href="/de/pagina" />
  <link rel="alternate" hreflang="nl" href="/nl/pagina" />
</head>
```
...
```bash
# Verifica se ainda há palavras alemãs comuns nos arquivos NL
grep -r " und \| der \| die \| das \| Sie \| nicht \| für \| mit " src/pages/nl/ --include="*.astro" 2>/dev/null

# Conta arquivos traduzidos vs total
echo "Arquivos DE:" && ls src/pages/de/ | wc -l
echo "Arquivos NL:" && ls src/pages/nl/ | wc -l
```

Confirme que:
- [ ] Todos os arquivos de `/de/` têm equivalente em `/nl/`
- [ ] Nenhum texto alemão visível permanece nas páginas NL
- [ ] `alt` e `aria-label` estão em holandês
- [ ] Metadados SEO estão em holandês
- [ ] `NL_IMAGES_TODO.md` foi gerado com todas as imagens a revisar
- [ ] `hreflang` está configurado corretamente para DE e NL
...
3. Trate as imagens com texto (Seção 3) — gere o relatório NL_IMAGES_TODO.md
...
*Briefing gerado por Oráculo Marketing — Projeto de localização DE → NL*
